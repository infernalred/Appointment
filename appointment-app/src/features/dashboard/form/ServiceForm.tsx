...
export default observer(function ServiceForm({id}: Props) {
    ...
    const validationSchema = Yup.object({
        title: Yup.string().required("Название обязательно").max(25, "Длина превышена"),
        durationMinutes: Yup.number().required("Продолжительность обязательна").when("title", (title, schema) => {
            return schema.test({
    ...
    useEffect(() => {
        if (id) loadService(id).then(service => setService(new ServiceFormValues(service)))
    }, [id, loadService])
