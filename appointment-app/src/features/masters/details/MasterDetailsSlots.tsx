    ...
    return (
        <div>
            {slots.length > 0 ? (
                <Fragment>
                    {slots.map(slot => (
                        <button className={selected?.id === slot.id ? "selected" : "buttonSlot"} disabled={selected && selected.id !== slot.id} onClick={() => handleSelected(slot)} key={slot.id}>{slot.start.toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit"
                        })}</button>
                    ))}
                ...
                <Fragment>
                    <button disabled className={"buttonSlot"}>Слотов нет</button>
                </Fragment>
            )}
        </div>
    );
